// ConsoleApplication_6.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int filter(const Item* items, int kolvo_items, Item* filter_items, double max_ves) {
    ...
    for (int i = 0; i < kolvo_items; i++) {
        if (items[i].group == 'A' && std::strncmp(items[i].items.A.material, "Derevo", 6) == 0 && items[i].items.A.ves <= max_ves) {
